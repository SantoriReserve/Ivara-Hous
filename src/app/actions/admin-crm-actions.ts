...
import { rethrowNextControlFlow } from "@/lib/admin/rethrow-next-control-flow";
...
export async function adminAddCustomerNoteAction(formData: FormData): Promise<AdminCrmActionResult> {
  try {
    ...
    if (!note) {
      return {
        success: false,
        error: "Could not save note. Apply migration 012 if tables are missing.",
      };
    }
    ...
    console.error("[admin] adminAddCustomerNoteAction failed:", error);
...
export async function adminDeleteCustomerNoteAction(
  ...
  try {
    ...
    rethrowNextControlFlow(error);
    console.error("[admin] adminDeleteCustomerNoteAction failed:", error);
    return { success: false, error: "Could not delete note." };
  }
}
...
export async function adminAddCustomerTagAction(formData: FormData): Promise<AdminCrmActionResult> {
  try {
    ...
    if (!created) {
      return {
        success: false,
        error: "Could not save tag. Apply migration 012 if tables are missing.",
      };
    }
    ...
    rethrowNextControlFlow(error);
    console.error("[admin] adminAddCustomerTagAction failed:", error);
    return { success: false, error: "Could not save tag." };
  }
}
...
export async function adminRemoveCustomerTagAction(
  ...
  try {
    ...
    rethrowNextControlFlow(error);
    console.error("[admin] adminRemoveCustomerTagAction failed:", error);
    return { success: false, error: "Could not remove tag." };
  }
}
