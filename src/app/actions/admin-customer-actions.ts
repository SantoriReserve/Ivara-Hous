  ...
  try {
    ...
    if (!userId) {
      return {
        success: false,
        error:
          "No Auth user exists for this email yet. Ask the customer to use Forgot Password on the site.",
      };
    }
    ...
    console.error("[admin] adminResendPasswordSetupAction failed:", error);
    return { success: false, error: "Could not send password setup email." };
  }
}
...
export async function adminResendWelcomeEmailAction(
  ...
  try {
    ...
    rethrowNextControlFlow(error);
    console.error("[admin] adminResendWelcomeEmailAction failed:", error);
    return { success: false, error: "Could not send welcome email." };
  }
}
